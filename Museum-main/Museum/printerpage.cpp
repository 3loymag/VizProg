...
PrinterPage::PrinterPage(CartManager *cartManager, QSqlDatabase *db, QWidget *parent)
    : QWidget(parent),
    ...
    connect(cartManager, &CartManager::itemsRemoved, this, &PrinterPage::restoreCountStore);
    connect(ui->cartButton, SIGNAL(clicked()), this, SLOT(onCartButtonClicked()));

}

void PrinterPage::onCartButtonClicked() {
    cartManager->exec();
    cartManager->updatePrices(printerPrices);
}

void PrinterPage::restoreCountStore(const QMap<QString, int> &itemsToRemove) {
...
void PrinterPage::loadPrintersFromDatabase() {
    ...
    QSqlQuery query("SELECT * FROM items WHERE category = 'Новосибирск'", *db);
    int widgetIndex = 0;
...
void PrinterPage::readPricesFromDatabase() {
...
void PrinterPage::readPrinter(QSqlQuery &query) {
    QString modelName = query.value("model").toString();
    double price = query.value("price").toDouble();
    QString image = query.value("photo_path").toString();
...
void PrinterPage::buyPrinter(const QString &itemName) {
...
void PrinterPage::buyPrinter(int index) {
