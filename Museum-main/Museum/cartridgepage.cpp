...
CartridgePage::CartridgePage(CartManager *cartManager, QSqlDatabase *db, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CartridgePage),
    ...
    for (int i = 0; i < 6; ++i) {
        int row = i / 3;
            ...
            QLineEdit* modelLineEdit = new QLineEdit(scrollWidget);
                ...
                // Подключаем события для кнопок покупки
    ...
    // Читаем цены из базы данных
    readPricesFromDatabase();
    connect(cartManager, &CartManager::itemsRemoved, this, &CartridgePage::restoreCountStore);
    connect(ui->cartButton, SIGNAL(clicked()), this, SLOT(onCartButtonClicked()));
}

void CartridgePage::onCartButtonClicked() {
    cartManager->exec();
    cartManager->updatePrices(printerPrices);
}

void CartridgePage::restoreCountStore(const QMap<QString, int> &itemsToRemove) {
...
void CartridgePage::loadPrintersFromDatabase() {
    ...
    QSqlQuery query("SELECT * FROM items WHERE category = 'Бердск'", *db);
    int widgetIndex = 0;
...
void CartridgePage::readPricesFromDatabase() {
    ...
    while (query.next()) {
        ...
        qDebug() << "Model:" << modelName << ", Price:" << price; // Add this line for debugging
...
void CartridgePage::readCatridge(QSqlQuery &query) {
    QString modelName = query.value("model").toString();
    double price = query.value("price").toDouble();
    QString image = query.value("photo_path").toString();
...
void CartridgePage::buyCatridge(const QString &itemName) {
    if (printerPrices.contains(itemName)) {
        ...
        if (widgetIndex != -1) {
            ...
            if (countStore > 0) {
                ...
                QMessageBox::information(this, "Упс.", "К сожалению эти билеты закончились. Выберите что-то другое или подождите поступление товара.");
            }
        } else {
